...
if (-e $file){
		...
		if (exists $datos{'tel'}){ $tel = $datos{'tel'};$tel =~ s/\n//g;$tel =~ s/://g;$tel =~ s/\///g;$sector =~ s/,//g;} else {$error == True};		
		if ((!$cuenta)||(!$nombre)||(!$DNI)||(!$sector)||(!$tel)|| $error)
...
#				$DNI = s/\.\-\s//g;
				system "$ejec '$server' 'cambiar.pl \"$nombre\"  \"$sector\" \"$tel\" \"$DNI\" $cuenta'";
				...
				$sess = $session[1];						
				$sess =~ s/\n//g;				
				print (LOGS $sess."   ".$date."   cambio cuenta: ".$cuenta."@".$server." para ".$DNI." desde  ".$ENV{'REMOTE_ADDR'}."\n");
				close (LOGS);};
		}
