...
if ((exists $hash_pas{$login})&&(($hash_pas{$login}) eq $encrypted))
						...
						close (SESSION); 
						my $dominios = $hash_registros{'dominios'};
                        $dominios =~ s/^\s*\(//g;
                        $dominios =~ s/\)\s*$//g;
                        @dom = split (/,/,$dominios);
                        my $selectdominio = "<SELECT NAME=\"selDominio\" id=\"dominio\" onchange=\"setdominio()\">\n";
                        foreach $linea (@dom){
                                my @tmp = split(/=/,$linea); # se separa el registo en campos
     	                        $selectdominio .= "<OPTION VALUE=\"$tmp[1]\">$tmp[0]</OPTION>\n";
     	                        };
                        $selectdominio .= "</SELECT>\n";
						my $dominio = (split(/=/,$dom[0]))[0];
						...
						foreach $linea (@lineas){
							...
							$linea =~ s/<SELECTDOMINIO>/$selectdominio/g;
     						print ("$linea\n");
     						};
						# Cerramos el fichero abierto
						close (PAGINA);}
